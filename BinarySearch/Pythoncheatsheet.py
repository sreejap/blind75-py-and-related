...
##
General Rules:
Use while left <= right: when you want to process every element, including when left == right. Common when you want to return the index of a found element or a boundary.
Use while left < right: when you want to shrink the search space to a single element, and then return left (or right). Common in boundary-finding problems (like lower_bound/upper_bound)
